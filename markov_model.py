...
def weighted_values(freq_dict):
	...
	for d in freq_dict:
		...
		for k in freq_dict[d].keys():
			print(freq_dict[d].keys())
			freq_dict[d][k] = freq_dict[d][k]/sum_freq
		return freq_dict
...
def random_key(freq_dict, pair_key):
	...
	dict_word = []
	dict_freq = []
	for word, freq in freq_dict[pair_key].iteritems():
		dict_word.append(word)
		dict_freq.append(freq)
	return np.random(dict_word, 1, p=dict_freq)[0]
...
class MarkovModel:
	...
	def generate_text(self, char_n):
		...
		while(n <= char_n):
			# add word to list
			word = random_key(self.frequency_dict, self.frequency_dict[(first, second)])
			markov.append(word)
...
mark_text = MarkovModel("Navy Seal")
mark_text.add_text(text)
mark_text.create_freq_dict()
freq = mark_text.frequency_dict
print(freq)
mark_text.generate_text(120)